    ...
    fun upsertJob(job: Job) {
        db.transaction { configuration ->
            ...
            databaseOperations.upsertJob(
                job.key, job.company.key, job.title, job.description, job.monthlySalary, job.equity, job.positionType, hasStar = false, isDeleted = false, poster = job.poster.filename)
